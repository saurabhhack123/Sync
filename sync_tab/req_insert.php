 ...
 /*recieve data from URI request */ 
  ...
  /* validating tab and prepare json data */
   ...
   if($is_data && $is_valid_req){
     ...
     $rows  = get_rows_for_insert_tab($table_name,$school_id,$tab_id);

     if($rows <= 3000){
       $offset = 1; 
       
       $response = _get_nsync_insert_data($school_id,$tab_id,$table_name,$offset);
