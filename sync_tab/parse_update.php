<?php
 ...
 /*  
  helper function used by req_update
 */
  ...
  function  is_data_for_tab($school_id,$tab_id){
    ...
    $sql  = "select * from sync_mgt where SchoolId='$school_id' and TabId='$tab_id' and Action='UPDATE' and IsAck=0 and Is_complex=0";
 ...
 function is_tab_valid($tab_id){
  ...
  function get_rows_for_update_tab($table_name,$school_id,$tab_id){
      $sql   = "select * from $table_name where SchoolId='$school_id' and TabId='$tab_id' and Action='UPDATE' and IsAck=0 and Is_complex=0";
...
//  /* ***********************************************/
  ...
  function _get_unique_tables($school_id,$tab_id,$table_name,$offset){
    ...
    $tables = array();
    $sql = "select distinct TableName from $table_name where SchoolId='$school_id' and TabId='$tab_id' and IsAck=0 and Action='UPDATE' and Is_complex=0 order by DateTimeRecordInserted ASC LIMIT $offset,3000";  
...
function create_arr_set($q_map){
  $set = array();
  foreach ($q_map["params"] as $key => $value) {
    $set[]=trim($key);
  }
  return $set;
}/* commenting to test new function
function create_arr_set($q_map){
  $set = array();
  foreach ($q_map["params"] as $key => $value) {
    $set[]=$key;
...
function create_arr_where($q_map){
  $where = array();
  foreach ($q_map["where"] as $key => $value) {
    $where[]=trim($key);
  }
  return $where;
}
...
function _prep_update_json($school_id,$tab_id,$table,$offset){
    ...
    $sql = "select Query,Sync_Mgt_Id,Map from sync_mgt where SchoolId='$school_id' and TabId='$tab_id' and IsAck=0 and Is_complex=0 and TableName='$table'   
                and Action='UPDATE' order by DateTimeRecordInserted ASC LIMIT $offset,3000";
    ...
    while($row = mysql_fetch_array($res)){
      ...
      $q_map     = json_decode($row["Map"],1); // converting to array object
         
      $group["ack_id"]=$ack_id;
      $group["set_keys"] = create_arr_set($q_map);
      $group["where_keys"] = create_arr_where($q_map);  
      $chunk["process"][] = $group;    
    }
   
 // {"params":{"Mark":"NULL "},"where":{" ActivityId":"134 "," StudentId":"12843 "," ExamId":"7787 "," SubjectId":"44"}}
   ...
   for($i=0;$i<count($help_chunk)-1;$i++){
      for($j=$i+1;$j<count($help_chunk);$j++){
            ...
            $is_set_keys   = $help_chunk[$i]["set_keys"]===$help_chunk[$j]["set_keys"];
            $is_where_keys = $help_chunk[$i]["where_keys"]===$help_chunk[$j]["where_keys"];

            if($is_set_keys && $is_where_keys)
              { 
   ...
   foreach ($final as $group_arr) {
          ...
          $grp_temp["update_col"]= create_arr_set($q_map);
          $grp_temp["where_col"] = create_arr_where($q_map);
          
          $grp_temp["update_val"] = array();
          ...
          foreach ($group_arr as $sync_mgt_id) {
               ...
               foreach($q_map["params"] as $p_key=>$p_val){
                   $temp[trim($p_key)]=trim(str_replace("'","",$p_val));
               }
               
               foreach($q_map["where"] as $q_key=>$q_val){
                   $temp[trim($q_key)]=trim(str_replace("'","",$q_val));
               }
               

               $temp["ack_id"] = $sync_mgt_id;
          
               $grp_temp["update_val"][] = $temp;  
          }
...
function _get_nsync_update_data($school_id,$tab_id,$table_name,$offset){
    $data["Sync"] = array();
     ...
     foreach($u_tables as $table){
        ...
        $update_arr = _prep_update_json($school_id,$tab_id,$table,$offset);
        
       /** json preparation end for updation */
        
        $data["Sync"] = array_merge($data["Sync"],$update_arr);
